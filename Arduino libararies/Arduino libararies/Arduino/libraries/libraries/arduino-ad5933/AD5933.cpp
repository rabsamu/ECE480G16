...
bool AD5933::calibrateAvg(double gain[], int ref, int n){
	int real1[n+1], imag1[n+1];
	//int *real1 = new int[n];
    //int *imag1 = new int[n];
	int real2[n+1], imag2[n+1];
	int real3[n+1], imag3[n+1];
	
	if (!frequencySweep(real1, imag1, n)) {
        return false;
    }
	if (!frequencySweep(real2, imag2, n)) {
        return false;
    }
	if (!frequencySweep(real3, imag3, n)) {
        return false;
    }
	...
	double imag;
	for (int i=0; i<n+1; i++) {
		real = (real1[i] + real2[i] + real3[i]) / 3;
		imag = (imag1[i] + imag2[i] + imag3[i]) / 3;
		gain[i] = (double)(1.0/ref)/sqrt(pow(real, 2) + pow(imag, 2));
	}
...
double AD5933::twoPointCalibration(int start_freq, int freq_incr, double ref, int num_incr, double linear_gain[]) {
    setIncrementFrequency(freq_incr);
    int r,i = 0;
	int cfreq = start_freq/1000; // kohm
	double gain_low, frq_low; // frq/gain calculated at lower frequency
	double gain_high, frq_high; // frq/gain calculated at higher frequency
	
	// Assume frq register is started, num incriments register is initialized, frq incriment register is initialized
	if (!(setPowerMode(POWER_STANDBY) &&         // place in standby
		setControlMode(CTRL_INIT_START_FREQ) && // init start freq
		setControlMode(CTRL_START_FREQ_SWEEP))) // begin frequency sweep
		{
		...
		{
                if ((readStatusRegister() & STATUS_SWEEP_DONE) != STATUS_SWEEP_DONE) {
					for (int j = 0; j < num_incr; j++, cfreq += freq_incr/1000) {
						if (!getComplexData(&r, &i)){						
							// Error occurs in reading
							r = 0;
							i = 0;
						} else {
							if (j==0) {
								gain_low = (double)(1.0/ref)/sqrt((r, 2) + pow(i, 2));
								frq_low = cfreq;
                                setControlMode(CTRL_INCREMENT_FREQ);
							} else if (j == num_incr-1) {
								gain_high = (double)(1.0/ref)/sqrt((r, 2) + pow(i, 2));
								frq_high = cfreq;
                                setControlMode(CTRL_INCREMENT_FREQ);
							}
						}
					}
					linear_gain[0] = (double)(gain_high-gain_low)/((frq_high-frq_low));
					linear_gain[1] = gain_low;
					return setPowerMode(POWER_STANDBY);
              }
